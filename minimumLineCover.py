...
def showGraph(G):
    ...
    node_colors = ["firebrick" if node in G.graph['unmatchedNodes']
                   else "lightcoral" for node in nodes]

    # show the edges in the matching, the edges in cover-line and the other edges with different colos
    edge_colors = []
    ["lightcoral" if G.nodes[u]['matchedWith'] == v and G.nodes[v]
        ['matchedWith'] == u else "lightblue" for (u, v) in edges]
    for (u, v) in edges:
    ...
    for u in U:
        ...
        for neighbor in neighbors:
            if G.nodes[neighbor]['matchedWith']:
                ...
                G.edges[(u, neighbor)]['color'] = "firebrick"
                showGraph(G)
                break
    print("S:", S)
    showGraph(G)
